...
if (!file.exists("sim_pop-escambia.tgz")) stop("File sim_pop-escambia.tgz must be in the same folder as this script.")
untar("sim_pop-escambia.tgz", exdir=".")

#### Connect
con <- dbConnect(RSQLite::SQLite(), "sim_pop-escambia/sim_pop-escambia.sqlite")
...
print(head(loc))
write_delim(loc, "locations-escambia.txt")
...
print(head(pers))
write_delim(pers, "population-escambia.txt")

## Simply copying the network files out
file.copy("sim_pop-escambia/network-escambia.txt", ".")

#### Finish and kill the unpacked files
message("Removing unpacked files...")
dbDisconnect(con)
unlink("./sim_pop-escambia", recursive = T)
message("Done.")
