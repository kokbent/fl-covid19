...
if (!file.exists("sim_pop-escambia-1.1.tgz")) stop("File sim_pop-escambia-1.1.tgz must be in the same folder as this script.")
untar("sim_pop-escambia-1.1.tgz", exdir=".")

#### Connect
con <- DBI::dbConnect(RSQLite::SQLite(), "sim_pop-escambia-1.1/sim_pop-escambia-1.1.sqlite")
...
print(head(loc))
fwrite(loc, "locations-escambia.txt", sep = " ")
...
print(head(pers))
fwrite(pers, "population-escambia.txt", sep = " ")
...
fwrite(hh_network, "network-escambia.txt", sep = " ", col.names = F)
...
print(head(ec))
fwrite(ec, "extracurricular-escambia.txt", sep = " ")

#### Finish and kill the unpacked files
message("Removing unpacked files...")
dbDisconnect(con)
unlink("./sim_pop-escambia-1.1", recursive = T)
message("Done.")
