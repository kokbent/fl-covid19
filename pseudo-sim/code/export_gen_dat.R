...
library(RSQLite)
library(stringr)
source("pseudo-sim/code/data_path.R")

#### Data import
wp <- fread("pseudo-sim/output/wp2.csv")
head(wp)
colnames(wp) <- tolower(colnames(wp))

hh <- fread("pseudo-sim/output/hh_coords.csv")
head(hh)
colnames(hh) <- tolower(colnames(hh))

nh <- fread("pseudo-sim/output/nh.csv")
head(nh)
...
sch <- fread("pseudo-sim/output/sch.csv")
head(sch)
...
hf <- fread("pseudo-sim/output/hf.csv")
head(hf)
...
pers <- fread("pseudo-sim/output/pers_w_wid.csv")
head(pers)
...
## HH Network
hh_edge <- fread("pseudo-sim/output/hh_network.csv")
hh_loc <- loc %>%
  filter(type == "h") %>%
  select(locid, hid)
...
## Neighbour Network
# nb_edge <- fread("pseudo-sim/output/neighbour_network.csv")
...
# ec <- fread("pseudo-sim/output/extracurricular.csv")
