...
ipums_hh <- dat %>%
  group_by(SERIAL, HHWT) %>%
  summarise(PPH = n()) %>%
  ungroup %>%
  summarise(PPH = sum(PPH * HHWT) / sum(HHWT))
# ipums_hh2 <- hh_dat %>%
#   group_by(PUMA) %>%
...
sc <- (sum(cenacs$TOTALPOP) / ipums_hh$PPH) / sum(cenacs$HOUSEHOLDS)
cenacs@data$scale <- sc
...
hh_samp <- hh_dat %>%
  sample_n(size = nrow(hh_xy_puma), 
           replace = T,
           weight = HHWT)

hh_xy_puma[,c("YEAR", "SERIAL")] <- hh_samp[,c("YEAR", "SERIAL")]
...
nh_pop <- person_xy %>%
  filter(GQ == 3, AGE >= 55)
nh <- read_csv("pseudo-sim/output/nh.csv")
