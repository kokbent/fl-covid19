...
} else if (!str_detect(arg, "sim_pop-.*-3.0.tgz")) {
  stop("Argument is not a valid sim_pop 3.0 tgz file.")
} else {
  tgz_file <- arg
}

#### Splicing args
cnt <- str_remove(tgz_file, "sim_pop-") %>% str_remove("-3.0.tgz")
...
loc <- loc %>%
  left_join(wp)
colnames(loc) <- c("locid", "x", "y", "type", "hfid", "compliance", "essential")
loc$essential[loc$type != "w"] <- "y"
loc$locid <- loc$locid - 1
loc$x <- round(loc$x, 5)
loc$y <- round(loc$y, 5)
loc <- loc %>%
  select(locid, x, y, type, essential, hfid, compliance)
