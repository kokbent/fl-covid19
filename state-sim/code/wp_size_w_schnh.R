...
source("state-sim/code/target_func.R")
...
## NH
if (file.exists("state-sim/output/nh.csv")) {
...
## SCH
if (file.exists("state-sim/output/sch.csv")) {
  sch <- fread("state-sim/output/sch.csv") %>%
...
## HF
if (file.exists("state-sim/output/hf.csv")) {
  hf <- fread("state-sim/output/hf.csv") %>%
...
fwrite(wp2, "state-sim/output/wp.csv")
